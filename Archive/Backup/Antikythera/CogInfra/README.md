# 认知基础设施 Cognitive Infrastructures
>https://coginfra.antikythera.org/
>
...
这代表了人工智能的基础设施化（infrastructuralization of AI），同时也是对新旧基础设施的“认知化（making cognitive）”过程。这些新系统能够以我们可识别为嵌入式（embedded）和网络化（networked）认知的方式，对我们、对世界以及彼此做出响应。人工智能被实体化（physicalized），从手持设备表面的用户界面（user interfaces）一直到建成环境（built environment）的深处。当我们与世界互动时，我们也在重新训练模型权重（model weights），使得行动因意识到执行一个行动也是在模型内部表征（representing）该行动的一种方式而变得具有新的反射性（reflexive）。与模型互动（play with the model）即是重塑模型，并且日益趋于实时。

这是一种什么样的设计空间（design space）？它提供了什么可能性（afford）、促成了什么（enable）、生产了什么（produce），又限定了什么（delimit）？当人工智能同时是平台（platforms）、应用程序（applications）和用户（users）时，社会与其智能模拟（intelligent simulations）之间的接口（interfaces）是什么？我们如何理解人工智能对齐（AI Alignment）不仅仅是人工智能屈从于社会，也包括社会如何在与人工智能的关系中演化？可能会揭示和构建出何种认知基础设施（Cognitive Infrastructures）？跨越不同尺度——从世界数据虚构（world-datafiction）和数据可视化（data visualization）到用户（users）和用户界面（UI），再返回——人工智能设计中许多最有趣的问题仍处于萌芽状态。
